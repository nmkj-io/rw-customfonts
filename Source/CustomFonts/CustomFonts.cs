...
namespace CustomFonts
{
    ...
    public static class FontPatcher
    {
        static FontPatcher()
        {
            if (FontSettings.CurrentFontName != FontSettings.DefaultFontName) CustomFonts.UpdateFont();
        }
    }
    ...
    public class CustomFonts : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect) // The GUI part to edit the mod settings.
        {
            ...
            string[] specimen =
            {
                ...
                !_isWindows && isDefaultFont ? "(The default font cannot render Japanese texts in non-windows environments.)" : "RimWorldは、知性のあるAIストーリーテラーによって織りなされるSFコロニーシミュレーションゲームです。"
            };
            ...
            if (listingStandard.RadioButton(FontSettings.DefaultFontName, isDefaultFont)) SaveFont(FontSettings.DefaultFontName);
